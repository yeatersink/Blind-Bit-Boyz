...
{#if results?.length > 0}
    ...
    <ul>
        {#each results as result}
            <li>{result.baseToken.name} ({result.baseToken.symbol}) {result.baseToken.address} / {result.quoteToken.name} ({result.quoteToken.symbol}) / {result.pairAddress} ${result.priceUsd}</li>
        {/each}
    </ul>
{/if}
